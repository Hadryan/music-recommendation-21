...
class NeuralCF(nn.Module):
  def __init__(self,num_users,num_songs,embedding_size,num_layers):
    super(NeuralCF,self).__init__()
    
    self.user_embedding_gmf = nn.Embedding(num_users,embedding_size)
    self.song_embedding_gmf = nn.Embedding(num_songs,embedding_size)
    self.user_embedding_mlp = nn.Embedding(num_users,embedding_size*(2**(num_layers-1)))
    self.song_embedding_mlp = nn.Embedding(num_songs,embedding_size*(2**(num_layers-1)))
